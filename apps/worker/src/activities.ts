...
function buildPrompt(input: ResearchInput): string {
  ...
  return [
    ...
    `EVERY thing you can think of. If you see it in a source, it's a`,
    `fact. Mint new predicates freely. 100-500 facts per session is`,
    `the target. An opinion with a source is as valuable as a birth date.`,
    `A GitHub repo description is 10+ facts (name, language, stars,`,
    `description, license, created date, last commit, topics, ...).`,
    `A resume entry is 5-10 facts. A blog post is 5-15 facts.`,
