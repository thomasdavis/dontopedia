export const BENCHMARK_EXTRACTION_PROMPT_V2 = `
...
These rules exist because we are building a traversable knowledge graph.
Violating them produces broken, unusable data.

### IRI formatting
- ALL IRIs must be kebab-lower-case: "ex:mrs-watson", NOT "ex:MrsWatson",
  NOT "ex:mrs_watson", NOT "ex:Mrs-Watson".
- Create specific, descriptive IRIs: "ex:monument-main-street-cooktown",
  NOT "ex:monument".

### Object values — NEVER use booleans
- NEVER use {"literal": {"v": true}} or {"literal": {"v": false}}.
  Booleans destroy information. The predicate already expresses the relationship.
- WRONG: (ex:cooktown, wasMunicipality, true)
- RIGHT: (ex:cooktown, wasA, {"iri": "ex:municipality"})
- WRONG: (ex:article, emphasizesHistory, true)
- RIGHT: (ex:article, emphasizes, {"iri": "ex:cooktown-municipal-history"})

### Object values — prefer IRIs over string literals
- When the object is an ENTITY (person, place, concept, event), use an IRI.
- Only use string literals for actual data values: names, dates, quotes, numbers.
- WRONG: (ex:cooktown, locatedIn, {"literal": {"v": "Queensland"}})
- RIGHT: (ex:cooktown, locatedIn, {"iri": "ex:queensland"})
- WRONG: (ex:article, framesAs, {"literal": {"v": "historical interest piece"}})
- RIGHT: (ex:article, framesAs, {"iri": "ex:historical-interest-narrative"})

### Object values — no string blobs
- String literals should be SHORT: a name, a date, a quote, a number.
- NOT a sentence. NOT a description. NOT an explanation.
- WRONG: {"literal": {"v": "unremitting care to her child when on Lizard Island"}}
- RIGHT: create an IRI for the concept: {"iri": "ex:unremitting-child-care-on-lizard-island"}

### Confidence calibration
- 1.0 = directly and explicitly stated in the text, zero inference needed
- 0.9 = very minor inference from explicit text
- 0.8 = moderate inference, reasonable but not certain
- 0.7 = significant inference, plausible interpretation
- 0.5-0.6 = speculative, reading between the lines
- Do NOT give 1.0 confidence to inferred facts. Reserve 1.0 for direct quotes
  and explicit statements only.

### Tier labelling honesty
- Article metadata (title, author, date, page) is ALWAYS Tier 1, never Tier 8.
- A presupposition must be something genuinely ASSUMED, not just stated.
- A Tier 7 philosophical claim must demonstrate genuine ontological analysis,
  not just a fancy predicate name on a Tier 1 fact.
- If you are unsure which tier, use the LOWER tier.

### Subject diversity
- Create MANY distinct subjects. A 500-word article should have 15-30+ subjects.
- Every person, place, event, concept, document, and institution deserves its own
  subject IRI with a (subject, name, "Human Label") fact.
- Separate the inscription from the person, the event from the report of the event,
  the monument from the heroism it commemorates.
